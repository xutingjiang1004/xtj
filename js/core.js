            ...
            (async function() {
                try {
                    ...
                    if (serverUser && serverUser !== currentUser) {
                        ...
                        // ★ 修复：refreshUserTokenViaCookie 对 401/403/网络错误从不 reject（返回空对象），
                        // 此前都没有分支处理：auth_pending 卡死、UI 维持"已登录"幽灵态、受保护操作
 ...
 */
            ...
            // ★ 修复：带超时的 fetch（半开连接/VPN 下裸 fetch 永不 settle 会卡死按钮与轮询）
            ...
            async function checkUserRestrictions() {
                ...
                try {
                    ...
                    var response = await fetchWithTimeout(API_BASE.replace(/\/$/, '') + '/api/user/restrictions', {
 ...
 */
            ...
            function repairProfileActivityText(value) {
                ...
                // ★ 修复：原字符类混入了 "ÃÂâ€œâ€" 这类损坏后入库的字面量；改为显式 Unicode 区间：
                // Latin-1 补充区(00A0-00FF) + Œ/œ/€ + 弯引号，覆盖常见的 UTF-8→Latin-1 乱码形态。
                if (/[\u00A0-\u00FF\u0152\u0153\u20AC\u2018\u2019\u201C\u201D\u201E]/.test(text) && !/[\u4e00-\u9fff]/.test(text)) {
                    try {
 ...
 */
        ...
        (function installFinalUiAndDataOverrides() {
            ...
            loadAnnouncements = async function(forceRefresh) {
                ...
                try {
                    ...
                    if (listEl && announcements && announcements.length) {
                        renderAnnouncementList();
                    } else if (listEl && Array.isArray(announcements) && announcements.length === 0 && listEl.children.length) {
                        // ★ 修复：服务端已清空公告时清掉旧列表，避免残留旧公告
                        listEl.innerHTML = '';
                    }
                } catch (e) {

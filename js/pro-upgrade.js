(function() {
  ...
  window.__xtjDirectPurchasePro = async function(userName) {
    ...
    if (!window.sb) return { ok: false, error: '数据库连接不可用' };
    ...
    try {
      ...
      if (error) {
        console.error('[Pro] Supabase insert failed:', error);
        return { ok: false, error: error.message || '数据库写入失败' };
      }
      ...
      console.error('[Pro] Direct purchase error:', e);
      return { ok: false, error: e.message || '激活失败' };
    }
  };

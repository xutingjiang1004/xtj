(function(){
  ...
  function saveLocalPhotoWallData(){
    var list = (Array.isArray(window.photoWallData) ? window.photoWallData : [])
      .filter(function(item){ return item && item.imageUrl && item.imageUrl.indexOf('data:') !== 0; })
      .slice(0, 180);
  ...
  function loadLocalPhotoWallData(){
    ...
    return Array.isArray(list) ? list.filter(function(item){ return item && item.id && item.imageUrl; }) : [];
  }
  ...
  function normalizePhotoWallRow(row){
    ...
    if (row.media_url === '__deleted__' || meta.__pw_del__ === true) {
      return { id: row.id, cloudId: row.id, imageUrl: null, deleted: true };
    }
    var url = row.media_url || meta.imageUrl || meta.url || '';
    var mime = meta.mimeType || row.mime_type || '';
    var kind = meta.mediaKind || meta.kind || (/^video\//i.test(mime) ? 'video' : 'image');
    return {
  ...
  async function fetchPhotoPage(pageIndex){
    ...
    var res = await window.sb.from('posts')
      ...
      .eq('media_type', window.PHOTO_WALL_MARKER || MARKER)
  ...
  async function loadPhotoWallData(force){
    ...
    try {
      ...
      if (!window.sb) {
        ...
        setPhotoWallSyncStatus(navigator.onLine === false ? 'offline' : 'error', navigator.onLine === false ? '离线缓存' : '连接异常');
      ...
      window.photoWallData = mergePhotoLists(cloud, local);
  ...
  async function deleteCloudPhoto(item){
    if (!item || !item.cloudId || !window.sb) return false;
    var payload = {
      photoId: item.cloudId,
      username: item.username,
      currentUser: window.currentUser || '',
      password_hash: localStorage.getItem('xtj_pw_hash') || ''
    };
    try {
      var api = await fetch('/api/photo/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (api.ok) {
        var json = await api.json().catch(function(){ return {}; });
        if (json.ok) return true;
      }
    } catch (_) {}
    try {
      var up = await window.sb.from('posts')
        .update({ content: JSON.stringify({ __pw_del__: true, ts: Date.now(), orig: item.username || '' }), media_url: '__deleted__' })
        .eq('id', item.cloudId);
      if (up && !up.error) return true;
    } catch (_) {}
    try {
      var del = await window.sb.from('posts').delete().eq('id', item.cloudId).select('id');
      if (del && !del.error) return true;
    } catch (_) {}
    return false;
  }
  ...
  async function deletePhotoWallPhoto(item, opts){
    ...
    var isAdmin = current === 'xxz';
    if (!current || (!isOwner && !isAdmin)) {
      toast('无权删除这张照片');
      return { ok:false, error:'unauthorized' };
    }
    var id = item.id || item.cloudId;
    try { await deleteCloudPhoto(item); } catch (err) { console.warn('[PhotoWall] cloud delete failed', err); }
  ...
  function handleExternalSync(message){
    ...
    if (message.type === 'photo_added') {
      setTimeout(function(){ loadPhotoWallData(true).then(function(){ if (typeof window.renderPhotoWallWithoutReload === 'function') window.renderPhotoWallWithoutReload(); }); }, 400);
    }
  }
    ...
    try {
      realtimeChannel = window.sb.channel('photo-wall-realtime')
        .on('postgres_changes', { event:'*', schema:'public', table:'posts', filter:'media_type=eq.' + (window.PHOTO_WALL_MARKER || MARKER) }, function(payload){
          ...
          loadPhotoWallData(true).then(function(){ if (typeof window.renderPhotoWallWithoutReload === 'function') window.renderPhotoWallWithoutReload(); });
        })

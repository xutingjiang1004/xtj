(function(){
  ...
  async function deleteCloudPhoto(item){
    ...
    // 1. 尝试 RPC 硬删除（带权限验证 + 返回 media_url 用于 Storage 清理）
    try {
      var rpcResult = await window.sb.rpc('delete_photo_wall_post', {
        p_post_id: id,
        p_username: window.currentUser || '',
        p_is_admin: (typeof window.isAdmin === 'function' && window.isAdmin()) || (window.currentUser === 'xxz')
      });
      if (rpcResult && rpcResult.ok && rpcResult.data && rpcResult.data.media_url) {
        // 2. 清理 Storage 中的文件
        var path = extractStoragePath(rpcResult.data.media_url);
        if (path) {
          try { await window.sb.storage.from('uploads').remove([path]); } catch (_) {}
        }
        return true;
      }
    } catch (_) {}

    // 降级1: 直接硬删除 + Storage 清理
    try {
      var mediaUrl = item.imageUrl || item.media_url || '';
      var storagePath = extractStoragePath(mediaUrl);
      if (storagePath) {
        try { await window.sb.storage.from('uploads').remove([storagePath]); } catch (_) {}
      }
      var del = await window.sb.from('posts').delete().eq('id', id);
      if (del && !del.error) return true;
    } catch (_) {}

    // 降级2: 软删除（兼容旧 RLS 策略不支持 DELETE）
    try {
      ...
      if (up && !up.error) {
        // 软删也清 Storage
        var sp = extractStoragePath(item.imageUrl || '');
        if (sp) { try { await window.sb.storage.from('uploads').remove([sp]); } catch (_) {} }
        return true;
      }
    } catch (_) {}
    return false;

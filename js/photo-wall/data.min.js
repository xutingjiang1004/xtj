(function(){
  ...
  async function deleteCloudPhoto(item){
    ...
    // 1. 优先用 RPC 硬删除（SECURITY DEFINER，校验权限）
    try {
      ...
      if (rpcResult && rpcResult.ok && rpcResult.data && rpcResult.data.media_url) {
        // 清理 Storage 文件
        var rpcPath = extractStoragePath(rpcResult.data.media_url);
        if (rpcPath) { try { await window.sb.storage.from('uploads').remove([rpcPath]); } catch (_) {} }
        return true;
      }
    } catch (_) {}

    // 2. 降级：直接硬删除 + Storage 清理
    try {

(function(){
  ...
  var BROKEN_KEY = 'xtj_photos_broken_media';
  ...
  var VIDEO_PLACEHOLDER = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#edfdf6"/><stop offset="55%" stop-color="#eaf6ff"/><stop offset="100%" stop-color="#dff4eb"/></linearGradient></defs><rect width="640" height="640" rx="44" fill="url(#bg)"/><circle cx="170" cy="160" r="90" fill="rgba(255,255,255,0.52)"/><polygon points="280,320 280,420 380,370" fill="#26a38a" opacity="0.82"/></svg>');
  ...
  function getBrokenIds(){
    try {
      var raw = localStorage.getItem(BROKEN_KEY);
      var list = raw ? JSON.parse(raw) : [];
      return Array.isArray(list) ? list.map(String) : [];
    } catch (_) { return []; }
  }

  function isBrokenPhotoWallPhoto(photo){
    return !!(photo && getBrokenIds().indexOf(String(photo.id)) >= 0);
  }

  function isRenderablePhoto(photo){
    if (!photo || isBrokenPhotoWallPhoto(photo)) return false;
    var url = photo.thumbUrl || photo.thumb || photo.imageUrl || '';
    return !!url;
  }
  ...
  function applyPhotoWallAspect(img){
    ...
    if (card && img && img.naturalWidth && img.naturalHeight) {
  ...
  function photoCardHtml(photos, startIndex){
    ...
    for (var i = 0; i < photos.length; i++) {
      ...
      var isVideo = p.mediaKind === 'video' || /^video\//.test(p.mimeType || '');
      var realUrl = p.thumbUrl || p.thumb || (isVideo ? VIDEO_PLACEHOLDER : p.imageUrl) || '';
      var username = p.username || '未知用户';
      var time = formatPhotoTime(p.timestamp);
      var index = base + i;
      var delay = Math.min(index * 30, 300);
      html += '<div class="photo-wall-item pw-stagger-enter" data-photo-id="' + esc(String(p.id)) + '" style="animation-delay:' + delay + 'ms" onclick="openPhotoPreview(' + index + ')">';
      html += '<img src="' + FALLBACK_IMG + '" alt="photo" class="pw-blur-in" data-src="' + safeUrl(realUrl) + '" loading="lazy" onerror="this.closest(\'.photo-wall-item\')&&(this.closest(\'.photo-wall-item\').style.display=\'none\')">';
      if (isVideo) html += '<span class="pw-video-badge">视频</span>';
  ...
  function albumHtml(groups){
    ...
    return groups.map(function(group){
      ...
      var isFirstVideo = first && (first.mediaKind === 'video' || /^video\//.test(first.mimeType || ''));
      var cover = first && (first.thumbUrl || first.thumb || (isFirstVideo ? VIDEO_PLACEHOLDER : first.imageUrl)) || FALLBACK_IMG;
  ...
  function pumpImages(){
    ...
    if (!url) { finishImg(img); return pumpImages(); }
    activeLoads++;
    img.onload = function(){ activeLoads--; img.removeAttribute('data-src'); finishImg(img); pumpImages(); };
    img.onerror = function(){ activeLoads--; img.src = FALLBACK_IMG; img.removeAttribute('data-src'); finishImg(img); pumpImages(); };
    img.src = url;
  ...
  function renderSorted(photos){
    ...
    if (!window.pwAlbumView) {
      ...
      if (!photos.length) { grid.innerHTML = emptyHtml(); return; }
  ...
  function installLoadMoreSentinel(grid){
    ...
    loadMoreObserver = new IntersectionObserver(async function(entries){
      for (var i=0;i<entries.length;i++) {
        ...
        if (!window.hasMorePhotos()) { sentinel.querySelector('.pw-load-more-indicator').textContent = '暂无更多'; continue; }
        loadingMore = true;
  ...
  async function renderPhotoWall(){
    ...
    rendering = false;
    if (pendingRender) { pendingRender = false; renderPhotoWall(); }
  }
  ...
  window.isBrokenPhotoWallPhoto = isBrokenPhotoWallPhoto;
  ...
  window.openPhotoAlbumGroup = function(key){ window.pwAlbumGroupKey = key || ''; renderPhotoWallWithoutReload(); };
  window.toggleAlbumView = function(){ window.pwAlbumView = !window.pwAlbumView; window.pwAlbumGroupKey = ''; renderPhotoWallWithoutReload(); };
  window.switchPhotoWallView = function(){ var sel = document.getElementById('pwAlbumSort'); window.pwSortKey = sel ? sel.value : (window.pwSortKey || 'date_desc'); renderPhotoWallWithoutReload(); };
  window.bindPhotoWallScroll = function(){ var h = document.querySelector('.photo-wall-header'); if (h) h.classList.remove('pw-header-hidden'); };
  window.updateAmbientBackground = function(url){ var bg = document.getElementById('ppAmbientBg'); if (bg && url) bg.style.background = 'radial-gradient(ellipse at center, rgba(100,160,140,.22) 0%, rgba(100,160,140,.06) 50%, transparent 80%)'; };
})();

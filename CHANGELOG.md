...
## 未发布 - 2026-09-23 安全审计修复
- `read_document` 改用 DNS 固定、拒绝重定向且限制大小的安全下载器，阻断 SSRF。
- 带会话 Cookie 的写请求缺少 Origin/可信 Referer 时一律拒绝，不再接受 `X-Requested-With` 绕过 CSRF。
- GitHub 写代理强制分支更新 `force: false`，限制 PATCH 仅更新分支，并拒绝写入 `.git` / `.github` 路径。
- Code 工作台校验 AI 写入路径，并要求所有多文件批量提交显式确认。
...
### 补完（同日第二轮，2026-09-22 晚）
上一轮的响应式修复只做了一半，本轮补齐并加测试基线：
- **横屏手机此前仍会套上桌面两列栅格**：`index.html` 的 `desktop.min.css` 只按 `(min-width: 768px)`
  加载，而 `desktop.css` 自身首条断点也是 768px，于是「宽 ≥768、高 ≈390」的横屏手机会得到
  “桌面栅格 + 移动外壳”的混合布局（`ui-shell.css` 侧对应规则已被 `min-height:480px` 门禁排除，
  两边不一致）。现加载条件补为 `(min-width: 768px) and (min-height: 480px)`，横屏手机真正落回移动布局。
- **断点留缝**：`ai-agent.css` 的 `max-width: 767px`、`ui-shell.css` 的 `max-width: 1279px` 与全局
  `768px` / `1280px` 的 `min-width` 规则之间留下 0.98px 空档（浏览器缩放/系统 DPI 缩放停在门槛附近时
  两边都不生效）。统一为 `767.98px` / `1279.98px`，与文件内既有 `.98` 风格一致。
- **还原 `css/desktop.css` 的 60 行乱码注释**：此前该文件正文注释被 UTF-8→GBK 双重编码破坏
  （如 `feed锛坒eed 鍐?1 列）`），上一轮只修了文件头。现已逐行还原为可读中文（仅注释，未动任何 CSS 声明）。
- **新增测试基线 `tests/ip-geo-localization-contract.test.js`（10 项）**：把属地本地化/并行竞速锁进契约
  ——`ipwho.is` 必须带 `lang=zh-CN`、必须是 `Promise.any` 并行竞速 + 整体截止、死源 `ip-api.com`
  不得回归、`/api/post/detail` 必须返回 `ip_region_text`，并对 `normalizeIpGeoName` 做行为验证
  （中文去行政后缀、英文按后缀剥离后映射、未命中保留原文、空值返回空串）。已登记进 `npm test`。
- **补记 08-11 ~ 09-21 的时间线**（见下），原文件从 v0.93.1 直接跳到 v0.94.1。

### 时间线补齐（2026-08-11 ~ 2026-09-21，这一段原本只字未提）

> 本文件此前停留在 `v0.93.1`（08-10），此后到 09-22 之间约 **175 个提交**没有任何词条。
> 下面按 git 实际提交补记主线（完整明细仍以 `git log` 为准）。

| 日期 | 提交数 | 主线 |
|---|---|---|
| 08-12 | 6 | 第二轮审计批量修复（P0/P1/P2） |
| 08-21 ~ 08-25 | 67 | 第三方自定义模型（多轮上下文/账号同步）、语音识别、**「思考 Max」档位**、Code 工作区（整库读取、分支合并）、HTML 分享包重生成 |
| 08-31 ~ 09-01 | 6 | 小猫 AI 评论修复、思考档位回归厂商原生参数、迁移 `052_cat_ai_hardening`、Code 工作区健壮性（P0-P3）、照片墙预览箭头触摸错位 |
| 09-02 ~ 09-03 | 17 | 代码审查真实 bug 与安全隐患修复、AI 思考球/文件读取、Code 工作区全文件读取（图片识图/附件）、仓库配置按账号隔离；**09-03 全量审计报告产出** |
| 09-04 | 1 | 应用 09-03 审计报告修复（S 级 10 + M 级 50+），迁移 `055_audit_fix_2026-09-04` |
| 09-11 | 1 | 深入研究无法对话/不显示结果修复；迁移 `054_quota_and_check_fixes` |
| 09-13 | 20 | 深入研究去 Tavily 硬依赖、**读网页 100% 失败（Node ≥20 `lookup` 契约）修复**（线上事故） |
| 09-14 ~ 09-15 | 11 | **工作模式**（AI 自主拆解任务 + 调用工具）+ 代码沙箱、读网页/中文 PDF/工具清单/搜索状态收敛修复、配额服务故障不再冒充「次数用尽」 |
| 09-17 ~ 09-18 | 4 | Flash 模型路由分裂、连接中断误报、工具调用多样性/速度、流式聊天「连接中断」误报、工具调用代码不再当正文泄露 |
| 09-21 | 4 | **客户端 IP 落成 Render 内网地址、属地解析全部失败**的修复（`trust proxy` 信任全部私有网段 + XFF 右往左公网兜底，回归锁定在 `tests/client-ip-resolution-contract.test.js`）；`+` 号面板卡死修复；迁移 `056_fix_increment_post_views_scope` |
| 09-22 | 29 | 本词条（v0.94.1）所记的静默失效修复、采集合规整改、属地/响应式；见 §十 修复记录明细 |

> 同日发现并修正一处**词条重号**：文件末尾另有一条 `## v0.93 - 2026-06-29 全面 Bug 审计修复 + 安全加固`，
> 与 08-10 的 `v0.93` 版本号重复。该条日期（06-29）落在 `v0.90`（06-25）与 `v0.93`（08-10）之间，
> 应为缺失的 `v0.91`/`v0.92` 之一，但无法从历史确定编号，故保留原文并加注说明（见文末该条标题）。
...
## v0.91/0.92（原标题误作「v0.93」，与 2026-08-10 的 v0.93 重号） - 2026-06-29 全面 Bug 审计修复 + 安全加固

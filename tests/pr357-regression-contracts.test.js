...
test('retry completed status returns full cat_ai comment data structure', () => {
  const retrySource = routeSource('post', '/api/comments/ai-reply-retry', "async function callDeepSeek(messages, options)");
  // B3：retry 完成分支统一走 fetchCatReplyByParent（内部按 cat_ai/generated_by_ai 过滤、select 完整字段、多行容错）
  assert.match(retrySource, /fetchCatReplyByParent\(commentId\)/);
  assert.match(retrySource, /aiReplyComment\s*=\s*__aiReplyQuery\.data/);
  // 统一查询函数本身必须保证 cat_ai 过滤与完整字段
  const fnStart = server.indexOf('async function fetchCatReplyByParent');
  assert.notEqual(fnStart, -1, '必须存在统一查询函数 fetchCatReplyByParent');
  const fnBlock = server.slice(fnStart, fnStart + 1200);
  assert.match(fnBlock, /\.eq\('user_name', CAT_AI_USERNAME\)/);
  assert.match(fnBlock, /\.eq\('generated_by_ai', true\)/);
  assert.match(fnBlock, /id,\s*post_id,\s*user_name,\s*content,\s*created_at[^)]*updated_at[^)]*parent_comment_id[^)]*generated_by_ai/);
});

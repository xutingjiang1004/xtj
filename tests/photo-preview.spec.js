...
async function setup(page) {
  ...
  await page.evaluate(() => {
    window.__listenerStats = { active: 0, max: 0, adds: 0, removes: 0, duplicateAdds: 0, unmatchedRemoves: 0 };
    window.__listenerSet = new Set();
    window.__listenerIds = new WeakMap();
    window.__listenerSeq = 0;
    const listenerKey = (target, type, listener) => {
      if (!window.__listenerIds.has(listener)) window.__listenerIds.set(listener, ++window.__listenerSeq);
      return target.id + ':' + type + ':' + window.__listenerIds.get(listener);
    };
    const add = EventTarget.prototype.addEventListener;
    const remove = EventTarget.prototype.removeEventListener;
    EventTarget.prototype.addEventListener = function(type, listener, options) {
      if (this && this.id === 'photoPreviewImage' && (type === 'load' || type === 'error') && listener) {
        const key = listenerKey(this, type, listener);
        if (window.__listenerSet.has(key)) window.__listenerStats.duplicateAdds += 1;
        else {
          window.__listenerSet.add(key);
          window.__listenerStats.adds += 1;
          window.__listenerStats.active = window.__listenerSet.size;
    ...
    EventTarget.prototype.removeEventListener = function(type, listener, options) {
      if (this && this.id === 'photoPreviewImage' && (type === 'load' || type === 'error') && listener) {
        const key = listenerKey(this, type, listener);
        if (window.__listenerSet.delete(key)) window.__listenerStats.removes += 1;
        else window.__listenerStats.unmatchedRemoves += 1;
        window.__listenerStats.active = window.__listenerSet.size;
      }
...
async function previewState(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      ppUrl: img && img._ppUrl,
      loadGen: img && img._ppLoadGen,
      placeholder: !!(img && img.classList.contains('pp-placeholder')),
...
test('main image error path has no pageerror and cleans open listeners', async ({ page }) => {
  ...
  expect(state.stats.duplicateAdds).toBe(0);
  expect(errors).toEqual([]);
});
...
test('failed retry from old URL cannot affect a newer image', async ({ page }) => {
  ...
  expect(state.ppUrl === null || state.ppUrl === '/ok2.png').toBe(true);

...
function buildMessages(input) {
  ...
  var questionInstruction = expectsQuestions
    ? '必须准确生成 question_count 对应的可作答题量：每个选择题计 1 题，每个完形填空 blank 计 1 题；总可作答题量必须严格等于 question_count，不能少题、零题或重复 id。'
    : '当前为 article-only 模式：questions 必须是空数组 []；不要生成选择题或完形填空；question_count 在该模式下不用于生成题目。';
  var instruction = [

...
-- 3) 表权限（给 anon 角色，PostgREST 用的是 anon）
...
-- ====== RPC 函数（sb.rpc 调用，绕过 PostgREST 表缓存）======

-- 5) 发送消息
create or replace function public.msg_send(
  p_sender text, p_receiver text, p_content text
) returns json as $$
declare
  v_id bigint;
begin
...
-- 6) 获取与某人的聊天记录
create or replace function public.msg_get(
  p_me text, p_other text
) returns table(
  id bigint, sender_name text, receiver_name text, content text, read boolean, created_at timestamptz
) as $$
begin
  return query
...
-- 7) 获取我的会话列表（每个对话对方最后一条）
create or replace function public.msg_convs(p_me text)
returns table(
  other_user text, last_message text, last_time timestamptz, unread integer
) as $$
begin
  return query
...
-- 8) 标记某人的消息为已读
create or replace function public.msg_mark_read(
  p_sender text, p_receiver text
) returns void as $$
begin
...
-- 9) 获取未读消息数
create or replace function public.msg_unread_count(p_receiver text)
returns integer as $$
declare
  v_cnt integer;
begin
  select count(*) into v_cnt
  from public.messages where receiver_name = p_receiver and read = false;
  return v_cnt;
end;
$$ language plpgsql security definer;
